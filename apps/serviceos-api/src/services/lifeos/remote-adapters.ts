...
export class RemoteTrustIdAdapter implements ITrustIdProvider {
  ...
  async issueDeliveryOtp(input: DeliveryOtpIssueInput): Promise<DeliveryOtpIssueResult> {
    return httpJson(this.baseUrl, "/v1/otp/delivery", {
      method: "POST",
      body: JSON.stringify(input),
    });
  }
  async verifyDeliveryOtp(input: { jobId: string; otpCode: string }): Promise<boolean> {
    const r = await httpJson<{ ok: boolean }>(this.baseUrl, "/v1/otp/delivery/verify", {
      method: "POST",
      body: JSON.stringify(input),
    });
    return r.ok;
  ...
  async verifyDriverLicense(input: DriverLicenseVerifyInput): Promise<DriverLicenseProof> {
    return httpJson(this.baseUrl, "/v1/identity/driver-license/verify", {
      method: "POST",
      body: JSON.stringify(input),
    });
  }
...
export class RemoteFundzManAdapter implements IFundzManWalletProvider {
  ...
  async transfer(payload: WalletTransferPayload): Promise<WalletTransferResult> {
    return httpJson(this.baseUrl, "/v1/wallet/transfer", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  }
  ...
  ): Promise<{ withdrawalId: string; status: "pending" | "settled"; amount: number }> {
    return httpJson(this.baseUrl, "/v1/wallet/withdraw", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  }
